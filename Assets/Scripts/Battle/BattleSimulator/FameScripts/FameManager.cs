...
public class FameManager
{
    ...
    private static Dictionary<int, BattleAgent> agentDictionary = new Dictionary<int, BattleAgent>();
    private static Dictionary<int, TroopAgent> flockDictionary = new Dictionary<int, TroopAgent>();
    ...
    public static void RegisterAgent(int agentID, BattleAgent member)
    {
    ...
    public static void RegisterFlock(int flockID, TroopAgent group)
    {
...
//        Debug.Log("remove agent of agentID:" + agentID + " " + hasItem);
...
//        Debug.Log("remove flock of flockID:" + flockID + " " + hasItem);
    ...
    public static void SetAgentAttribute(int agentID, Battle.BattleAgent.AgentAttribute attribute, float value)
    {
        FAME.Singleton.SetAgentAttributeAgent(agentID, System.Enum.GetName(typeof(Battle.BattleAgent.AgentAttribute), attribute), value);
    }
    ...
    /// <param name="attribute"></param>
    /// <param name="value"></param>
    public static void SetAgentAttributeGroup(int flockID, Battle.BattleAgent.AgentAttribute attribute, float value)
    {
        FAME.Singleton.SetAgentAttributeGroup(flockID, System.Enum.GetName(typeof(Battle.BattleAgent.AgentAttribute), attribute), value);
    }
    ...
    public static TroopAgent GetFlockGroup(int flockID)
    {
    ...
    public static BattleAgent GetFlockMember(int agentID)
    {
    ...
    /// <returns></returns>
    public static TroopAgent CreateFlock(Vector3[] shape, FlockType flockType, Type flockComponentType, float meshHeight = 10, int[] agents = null, TroopAgent group = null)
    {
        if (typeof(TroopAgent).IsAssignableFrom(flockComponentType))
        {
            ...
            if (agents != null)
            {
                ...
                {
                    BattleAgent member = GetFlockMember(agents[i]);
...
//        }
    ...
    public static TroopAgent CreateFlock(Type flockComponentType, FlockType flockType, int[] agents, TroopAgent group)
    {
        if (agents.Length == 0)
        {
        ...
        if (typeof(TroopAgent).IsAssignableFrom(flockComponentType))
        {
            ...
            {
                BattleAgent member = GetFlockMember(agents[i]);
                agentPosition[i] = MathUtil.ParseToVector3(member.Position);
            }
            if (agents.Length == 1)
            {
    ...
    public static void MoveAgent(int agentID, Vector3 position)
    {
        ...
        BattleAgent member = GetFlockMember(agentID);
        if (member.FlockID == -1)
        {
    ...
    public static void AddSteeringBehaviorAgent(int agentID, TroopAgent.SteeringBehaviors behavior, float radius, float weight)
    {
        FAME.Singleton.AddSteeringBehaviorAgent(agentID, System.Enum.GetName(typeof(TroopAgent.SteeringBehaviors),
    ...
    /// </summary>
    /// <param name="agentID"></param>
    public static void RemoveSteeringBehaviorAgent(int agentID, TroopAgent.SteeringBehaviors behavior)
    {
        FAME.Singleton.RemoveSteeringBehaviorAgent(agentID, System.Enum.GetName(typeof(TroopAgent.SteeringBehaviors), behavior));
    }
    /// <summary>
    /// Internal function. Call this in flockGroup or flockMember instead
    /// </summary>
    /// <param name="agentID"></param>
    public static void EditSteeringBehaviorAgent(int agentID, TroopAgent.SteeringBehaviors behavior, float radius, float weight)
    {
        FAME.Singleton.EditSteeringBehaviourAgent(agentID, System.Enum.GetName(typeof(TroopAgent.SteeringBehaviors), behavior), radius, weight);
    }
    /// <summary>
    ...
    [SerializeField]
    private bool getInfoFromUnityTerrain = false;
	...
	public void Tick(float deltaTime)
    {
		...
		FAME.Singleton.Update(deltaTime);

        foreach(KeyValuePair<int, BattleAgent> kvp in agentDictionary){
    ...
    {
		FameCSharp.FameSettings.accelerationThreshold = 1;//accelerationThresh100old;
        FameCSharp.FameSettings.ApplyTerrainEffect = false;//applyTerrainEffect;
        FameCSharp.FameSettings.slowingDistance = 1;//slowingDistance;
        FameCSharp.FameSettings.ignoreMinSpeed = 0;//ignoreMinSpeed;
        FameCSharp.FameSettings.SwitchAgentPosition = false;//switchAgentPosition;
...
//				}
//			}
			Vector3 pos = new Vector3(terrainWidth * -0.5f, 0, terrainLength * -0.5f);
