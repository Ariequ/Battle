...
public class CommonPSDImporter : Editor
{
    ...
    private static bool isCommonPSD;
...
//		canvasScaler.referenceResolution = new Vector2 (psdUI.psdSize.width, psdUI.psdSize.height);
        ...
        if (layer.images != null)
        {
            ...
            for (int imageIndex = 0; imageIndex < layer.images.Length; imageIndex++)
            {
                ...
                if (image.imageSource == PSDUI.ImageSource.Custom)
                {
                    ...
                    if (texturePathName != targetPathName && File.Exists (texturePathName))
                    {
                        if (File.Exists (targetPathName) == true)
                        {
                            Debug.Log (targetPathName + " already exists!!!!!! the old one will be replaced");
            ...
            for (int imageIndex = 0; imageIndex < layer.images.Length; imageIndex++)
            {
                ...
                if (image.imageSource == PSDUI.ImageSource.Custom)
                {
                    ...
                    if (isCommonPSD)
                    {
                        if (layer.name == PSDImporterConst.IMAGE)
                        {
                            pluginPath = PSDImporterConst.IMAGE + "/";
                        }
                        else
                        if (layer.name == PSDImporterConst.NINE_SLICE)
                        {
                            pluginPath = PSDImporterConst.NINE_SLICE + "/";
                        }
                    }

                    string assetPath = baseDirectory + pluginPath + image.name + PSDImporterConst.PNG_SUFFIX;
...
//                        myText.font = image.arguments[1];
                    ...
                    if (image.imageType == PSDUI.ImageType.Texture)
                    {
                        Image pic = Resources.Load (PSDImporterConst.PREFAB_PATH_IMAGE, typeof(Image)) as Image;
                        pic.sprite = null;
                        Image myImage = GameObject.Instantiate (pic) as Image;
